...
import { getAllPacks, orderPackages, setFilterPacksByStars } from "../../redux/actions/packageActions";
//icons
...
export default function PackageCardContainer() {
  ...
  return (
    <>
      ...
      <Card>
        <Grid display="flex" flexDirection="row" sx={{width: "15vw", marginLeft: "2%"}}>
            <Grid>
              <IconButton onClick={() => handleOrder("price")}>{objOrder[orderPrice]}</IconButton>
              <Tooltip title="ordenar por precio">
                <IconButton disableRipple onClick={() => handleOrder("price")}><AttachMoneyIcon/></IconButton>
              </Tooltip>
            </Grid>
            <Grid>
              <IconButton onClick={() => handleOrder("duration")}>{objOrder[orderDuration]}</IconButton>
              <Tooltip title="ordenar por duracion">
                <IconButton disableRipple onClick={() => handleOrder("duration")}><AvTimerIcon/></IconButton>
              </Tooltip>
            </Grid>
            <Grid>
              <IconButton onClick={() => handleOrder("rating")}>{objOrder[orderRating]}</IconButton>
              <Tooltip title="ordenar por rating de los usuarios">
                <IconButton disableRipple onClick={() => handleOrder("rating")}><GradeIcon/></IconButton>
              </Tooltip>
            </Grid>
        </Grid>
        <CardContent>
