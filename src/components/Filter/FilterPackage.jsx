...
export default function FilterPackage({ handleSubmit }) {
  ...
  const validation = () => {
    ...
    if (maxPrice && minPrice >= maxPrice)
      errorsCheck.minPrice = 'el precio minimo no puede ser mayor al maximo';
    if (minPrice && minPrice < 0)
      errorsCheck.minPrice = 'el precio minimo no puede ser menor que 0';
    if (maxPrice && maxPrice < 0)
      errorsCheck.maxPrice = 'el precio maximo no puede ser 0';
    if (minDuration && (minDuration < 1 || minDuration > 60))
      errorsCheck.minDuration = 'nada menor a 1 ni mayor a 60';
    if (maxDuration && maxDuration < 2)
      errorsCheck.maxDuration = 'la duracion maxima no puede ser menor que 2';
    if (maxDuration && minDuration > maxDuration)
      errorsCheck.minDuration =
        'la duracion minima no puede ser mayor que la maxima';
    if (
      filterByDateFin &&
      filterByDateInit &&
      filterByDateFin.getTime() - filterByDateInit.getTime() <= 0
    )
      errorsCheck.filterByDateInit =
    ...
    >
      <Card>
        <CardHeader
          ...
          action={
            <Button onClick={() => setOpen(!open)} aria-label='expand'>
              {open ? 'cerrar' : 'abrir'}
            </Button>
          }
        ></CardHeader>
                ...
                <Grid item>
                  <Grid>
                    ...
                    <Typography variant='h5'>
                      fecha de inicio:{' '}
                      {filters.filterByDateInit &&
                        filters.filterByDateInit.toLocaleString().split(',')[0]}
                    </Typography>
                    <Typography variant='h5'>
                      fecha de fin:{' '}
                      {filters.filterByDateFin &&
                        filters.filterByDateFin.toLocaleString().split(',')[0]}
                    </Typography>
                  </Grid>
                  <Grid
                    display='flex'
