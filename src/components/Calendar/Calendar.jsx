...
export default function Calendar({handleClick}) {
    ...
    const renderDays = () => {
        ...
        const fecha = new Date(`${meses[months[currentMonth]]}//1//${year[currentYear]}`);
        const firstDay = fecha.getDay();
        const previousMonthDays = daysPerMonth[months[currentMonth? currentMonth-1 : 11]];
        let dia = 1;
                    ...
                    if (elem === 0) {
                        if (firstDay > 0) {
                            ...
                            <Box display="flex" flexDirection="row" justifyContent="space-evenly">
                                {mapeadorDias.map(() => {
                                        return <Button key={"greyBI" + referenciaInicio} type="button" size="small" disabled>{previousMonthDays - 6 + referenciaInicio++}</Button>
                                })}
                            </Box>
                        )
                    } else {
            ...
            <Box>
                <Box display="flex" flexDirection="row" justifyContent="space-between">
                    <Box display="flex" flexDirection="row" marginTop="0.5%" marginLeft="1%">
                        <Typography variant="h6">{months[currentMonth]} {year[currentYear]}</Typography>
                    </Box>
                    <Box>
                        <Button disableRipple onClick={() => handleChange("zurdo")} type="button" size="large"><KeyboardArrowLeftIcon/></Button>
                        <Button disableRipple onClick={() => handleChange("derecho")} type="button" size="large"><KeyboardArrowRightIcon/></Button>
                    </Box>
                </Box>
                <Box>
                    <Box display="flex" flexDirection="row" justifyContent="space-evenly">
