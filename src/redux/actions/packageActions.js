...
import {
  ...
  getPackageName,
  getOrderedPackages,
} from '../reducer/packageSlice';

export const getAllPacks = () => async (dispatch) => {
  try {
    axios('/package').then((res) => dispatch(getAllPackages(res.data)));
  } catch (error) {
...
export const getFilterPacks =
  (filters, packages, stars) => async (dispatch) => {
    try {
      const response = await axios.post(`/package/filter`, {
        filters,
        packages,
        stars,
      });
...
export const orderPackages = (order, type, packages) => (dispatch) => {
  if (packages) {
    ...
    if (type === 'price') {
      ...
      if (order === 'descendent')
        toBeOrdered.sort((a, b) => b.duration - a.duration);
      if (order === 'ascendent')
        toBeOrdered.sort((a, b) => a.duration - b.duration);
    } else {
      if (order === 'descendent')
        toBeOrdered.sort((a, b) => b.rating - a.rating);
      if (order === 'ascendent')
        toBeOrdered.sort((a, b) => a.rating - b.rating);
    }
